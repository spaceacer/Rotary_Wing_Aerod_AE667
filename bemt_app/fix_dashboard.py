...
# I need to add import plotly.graph_objects as go
if 'import plotly.graph_objects as go' not in content:
    content = content.replace('import numpy as np', 'import numpy as np\nimport plotly.graph_objects as go')

start_idx = content.find("# ── build figure")
if start_idx == -1:
    print("Cannot find # ── build figure")
    exit(1)

end_idx = content.find("with tab_inspect:")
if end_idx == -1:
    print("Cannot find with tab_inspect:")
    exit(1)

head = content[:start_idx]
tail = content[end_idx:]

new_middle = """# ── build figure ──────────────────────────────────────────────────────────────
...
new_content = head + new_middle + tail
